...
from AlphaOP._misc._assistant import callback, in_pattern
from AlphaOP.dB._core import HELP, LIST
from AlphaOP.fns.helper import gen_chlog, time_formatter, updater
from AlphaOP.fns.misc import split_list
...
SUP_BUTTONS = [
    [
        Button.url("• Repo •", url="https://github.com/Light-Sensei/AlphaUB"),
...
async def inline_handler(event):
    ...
    if inline_pic():
        ...
        result = await event.builder.article(
            title="ALPHA Help Menu", text=text, buttons=_main_help_menu
        )
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("ALPHA Op", text=txt, link_preview=False, buttons=btn)
    ]
